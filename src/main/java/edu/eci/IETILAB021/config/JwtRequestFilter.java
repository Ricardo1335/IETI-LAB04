package edu.eci.IETILAB021.config;
...
import static edu.eci.IETILAB021.data.Constants.COOKIE_NAM;
            ...
            try
            {
                Optional<Cookie> optionalCookie =
                    request.getCookies() != null ? Arrays.stream( request.getCookies() ).filter(
                        cookie -> Objects.equals( cookie.getName(), COOKIE_NAM ) ).findFirst() : Optional.empty();
